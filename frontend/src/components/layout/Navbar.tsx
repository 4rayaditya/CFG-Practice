...
export const Navbar: React.FC = () => {
  ...
  const getRoleBadge = (role: string) => {
    switch (role) {
      case 'admin':
        return { label: 'Admin Director', bg: 'bg-purple-100 text-purple-800 border-purple-200', icon: Shield };
      case 'mentor':
        return { label: 'Volunteer Mentor', bg: 'bg-emerald-100 text-emerald-800 border-emerald-200', icon: Users };
      case 'student':
      default:
        return { label: 'Student', bg: 'bg-sky-100 text-sky-800 border-sky-200', icon: GraduationCap };
    }
  };
          ...
          <div className="hidden lg:flex items-center gap-3">
            {isAuthenticated && user ? (
              ...
              <div className="flex items-center gap-2">
                <Link
                  to="/login"
                  className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-xs font-semibold text-slate-700 hover:text-slate-900 hover:bg-slate-100 transition"
                >
                ...
                <Link
                  to="/register"
                  className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-semibold bg-sky-600 hover:bg-sky-500 text-white shadow-sm transition"
                >
