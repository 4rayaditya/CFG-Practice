...
| --- | --- | --- | --- |
| **Role-Based Auth** | P0 | Secure login for Students, Mentors, and Admins with route protection (React Router AuthGuards + Backend JWT Middleware + Supabase RLS). | Person 1 & 3 |
| **Voice-to-Text AI** | P0 | Audio capture UI that posts to a backend API endpoint integrating OpenAI Whisper for transcription. | Person 2 & 3 |
...
| **Offline Syncing** | P0 | PWA manifest, Service Workers (`vite-plugin-pwa`), and IndexedDB to cache failed audio uploads and auto-sync when online. | Person 4 |
| **Live Doubt Board** | P1 | Real-time Supabase `supabase-js` client subscriptions updating the Q&A feed seamlessly on the frontend. | Person 4 |
| **Career Roadmap** | P1 | Backend API endpoint using GPT-4o-mini to generate structured milestone roadmaps saved as interactive UI checklists. | Person 2 & 3 |
| **Admin Analytics** | P1 | Recharts dashboard showing resolution speeds and active users, populated by seeded data via the backend API. | Person 3 |

## 5. Technical Architecture & Stack

* **Frontend:** React SPA (Vite), TypeScript, React Router, Tailwind CSS, Lucide Icons, Recharts.
* **Backend:** Python (FastAPI) or Node.js (Express), JWT Authentication.
